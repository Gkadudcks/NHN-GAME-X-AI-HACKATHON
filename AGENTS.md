...
## External AI review routing

- 사용자가 외부 리뷰, 웹 리뷰, 평가단, 교차검증, 다른 AI 의견, GPT·ChatGPT 리뷰, Gemini·제미나이 리뷰를 요청하면 `external-ai-review` 스킬을 사용한다.
- "외부 리뷰"는 기본적으로 단일 외부 모델 리뷰를, "평가단"과 "교차검증"은 가능한 경우 ChatGPT 웹과 Gemini 웹의 독립 평가 후 Codex가 근거를 검증·종합하는 절차를 의미한다.
- 사용자가 공급자나 읽기 전용 범위를 지정하면 그대로 따른다. 외부 모델의 의견은 검증 전 코드에 반영하지 않는다.
