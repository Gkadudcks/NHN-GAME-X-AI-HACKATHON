...
- 장애물 판정은 점프 착지·슬라이드 종료를 포함해 도윤과의 충돌 가능성이 끝날 때까지 유지한다. 도윤 뒤로 넘어간 시각 표현은 하린과 도윤 사이에서 잘라내 동료 실루엣과 겹치지 않게 한다.
...
- 일상적인 점프·슬라이드 입력과 장애물 회피 성공은 별도 문구 없이 캐릭터 포즈와 버튼 반응으로만 전달한다. 피격·하린 보조·수집·회복 피드백은 유지한다.
...
| 2026-08-07 | 후방 장애물·성공 문구 보정 | 판정 수명은 유지하고 하린 앞에서 시각 차단, 일상 행동·회피 성공 문구 제거 |
