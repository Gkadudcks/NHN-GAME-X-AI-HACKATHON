...
상태: **CUE·렌더링·고정 대형 개선 완료 · 남은 작업 2개: 4프레임 달리기 → 최종 통합**
...
신규 또는 재가공 이미지는 사용자 승인 전 production에 적용하지 않는다. 완료된 작업은 `[AUTHORITATIVE]` 문서의 압축 완료표에 기록돼 있으며, 현재 남은 순서는 `4프레임 달리기 → 최종 통합`이다.
