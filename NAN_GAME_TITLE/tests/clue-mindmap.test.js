...
test('단서 보드는 원형 노드와 곡선 연결선으로 구성하고 드래그 이동을 지원한다', () => {
  ...
  assert.match(source, /initialScaleMultiplier: selectedDay \? 1\.22 : 1/);
});
...
test('추론에 쓰이지 않는 일반 업무 기록은 기존 저장 데이터에서도 제거한다', () => {
  const mindmap = loadMindmap();
  const clues = mindmap.pruneClues([
    '조사 자료: 경쟁 게임 사례',
    'PT 방향: 튜토리얼 단축',
    '박태식의 최초 지시: 유저 경험 중심·검증된 수치 사용',
    '과거 폴더의 비활성 자동화 — 상태: 비활성',
  ]);
  assert.deepEqual(Array.from(clues), [
    '박태식의 최초 지시: 유저 경험 중심·검증된 수치 사용',
    '과거 폴더의 비활성 자동화 — 상태: 비활성',
  ]);
});
