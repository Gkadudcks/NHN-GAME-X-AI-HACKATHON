...
test("자동저장 관련 정적 자산 캐시 버전이 모든 진입 페이지에서 갱신된다", () => {
  ...
  assert.match(index, /progress-store\.js\?v=5/);
  assert.match(index, /title-screen\.js\?v=15/);
  for (const html of [game, day2, day3]) {
    assert.match(html, /game\.css\?v=28/);
    assert.match(html, /progress-store\.js\?v=5/);
    assert.match(html, /settings-dialog\.css\?v=2/);
    assert.match(html, /settings-store\.js\?v=3/);
    assert.match(html, /settings-dialog\.js\?v=2/);
  }
  assert.match(game, /game\.js\?v=35/);
  assert.match(day2, /day2\.js\?v=25/);
  assert.match(day3, /day3\.js\?v=23/);
});
